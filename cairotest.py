...
# Welcome to the Cairo smart test framework
#
...
class SmartTest:
    ...
    def __init__(self, compiled_file=None, filename=None):
        if (filename == None) and (compiled_file != None):
            # We have a test object?  Wrap it in a CairoTest and load it into self.test_object
            self.test_object = CairoTest(compiled_file)
        elif (filename != None) and (compiled_file == None):
